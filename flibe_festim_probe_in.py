...
def build_festim_model(
    ...
    u = htm.ureg
    solubility_Nb = htm.solubilities.filter(material=htm.NIOBIUM)[
        0
    ]  # only one paper and it's for H in Nb
    ...
    # probe material parameters -- Nb
    Nb = F.Material(
        D_0=diffusivity_Nb.pre_exp.magnitude,
        E_D=diffusivity_Nb.act_energy.magnitude,
        K_S_0=solubility_Nb.pre_exp.magnitude,
        E_K_S=solubility_Nb.act_energy.magnitude,
    )
    ...
    probe = F.VolumeSubdomain(id=probe_marker, material=Nb)
    ...
    vacuum_surface_reaction_h2 = F.SurfaceReactionBC(
        ...
        k_r0=recombination_Nb.pre_exp.magnitude,  # * 1e25,
        E_kr=recombination_Nb.act_energy.magnitude,
...
if __name__ == "__main__":
    ...
    my_model = build_festim_model(
        openfoam_data_file="OpenFOAM/benchmark_cases_FLiBe/laminar/probe.foam",
        openfoam_final_time=290,
